...
function fStartPop(startc,endc) {
  ...
  if (!sd) sd=gEnd;
	gBegin[0] = new Date().getFullYear();
 	gBegin[1] = new Date().getMonth()+1;  
	gBegin[2] = new Date().getDate();
...
function fEndPop(startc,endc) {
  ...
  if (!sd) sd=gBegin;
 if(sd[1] > 9) {
		fPopCalendar(endc, [sd,[sd[0]+1,sd[1],sd[2]],sd]);  }
if(sd[1] <= 9 ) 
  fPopCalendar(endc, [sd,[sd[0],sd[1]+3,sd[2]],sd]);  //[2009,12,31] ====> gBegin
}
